...
class ECALayer(nn.Module):
    ...
    def forward(self, x):
        avg_pool = torch.mean(x, dim=[2, 3], keepdim=True)  # Global average pooling (N, C, 1, 1)
        avg_pool = avg_pool.view(avg_pool.size(0), avg_pool.size(1), -1)  # (N, C, 1)
        attention = self.conv(avg_pool)  # (N, C, 1)
        attention = self.sigmoid(attention)  # Normalize to [0, 1]
        attention = attention.view(attention.size(0), attention.size(1), 1, 1)
        return x * attention  # Apply the attention weights
